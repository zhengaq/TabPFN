...
estimators = [1, 2]
ignore_pretraining_limits_bool = [True, False]
optimization_spaces = ["raw_label_space", "preprocessed"]
...
all_combinations = list(
    product(
        ...
        ignore_pretraining_limits_bool,
        optimization_spaces,
    )
)
...
def synthetic_regression_data():
    ...
    y += rng.normal(size=100).astype(np.float32) * 0.1
    return X, y
