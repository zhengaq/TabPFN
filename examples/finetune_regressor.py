...
def prepare_data(config: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    bike_sharing = sklearn.datasets.fetch_openml(name="Bike_Sharing_Demand", version=2, as_frame=True, parser='auto')
    ...
    splitter = partial(train_test_split, test_size=config["test_set_ratio"], random_state=config["random_seed"])
...
def evaluate_regressor(regressor: TabPFNRegressor, eval_config: dict, X_train: np.ndarray, y_train: np.ndarray,
...
def main():
    ...
    training_datasets = regressor.get_preprocessed_datasets(X_train, y_train, splitter,
                                                            max_data_size=config["finetuning"]["batch_size"])
    finetuning_dataloader = DataLoader(training_datasets, batch_size=config["finetuning"]["meta_batch_size"],
    ...
    print(f"--- Optimizer Initialized: Adam, LR: {config['finetuning']['learning_rate']} ---\n")
    ...
    for epoch in range(config["finetuning"]["epochs"] + 1):
        if epoch > 0:
            ...
            for data_batch in progress_bar:
                optimizer.zero_grad()
                (
                    X_trains_p, X_tests_p, y_trains_p, y_test_std, cat_ixs, confs,
                    norm_bardist, bardist, _, batch_y_test_raw
                ) = data_batch
        ...
        mse, mae, r2 = evaluate_regressor(regressor, eval_config, X_train, y_train, X_test, y_test)

        status = "Initial" if epoch == 0 else f"Epoch {epoch}"
        print(f"📊 {status} Evaluation | Test MSE: {mse:.4f}, Test MAE: {mae:.4f}, Test R2: {r2:.4f}\n")
